...
public class FarmService {
    ...
    private static EmployeeData initEmployee() {
        ...
        int empTypeVal = InputService.readInt(
                Employee.EmployeeType.getAll() + "Select employee time: ",
                "This option does not exist. Try again: ",
                1, Employee.EmployeeType.values().length
        );
        Employee.EmployeeType auxEmpType = Employee.EmployeeType.getEmployeeType(empTypeVal);

        return new EmployeeData(auxSalary, auxWorkShift, auxEmpType);
    }

    public static AnimalCaretaker initAnimalCare() {
        System.out.println("Let's create an animal caretaker!");
        PersonData personData = initPerson();
        EmployeeData employeeData = initEmployee();
